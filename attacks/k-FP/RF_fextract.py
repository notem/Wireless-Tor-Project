...
# trace file columns are split at this character string
DELIMITER = "\t"
...
def pkt_concentration_stats(trace_data):
    ...
    stdconc = np.std(concentrations)
    avgconc = sum(concentrations) / float(len(concentrations))
    medconc = np.percentile(concentrations, 50)
    minconc = min(concentrations)
    maxconc = max(concentrations)
    return stdconc, avgconc, medconc, minconc, maxconc, concentrations
...
def TOTAL_FEATURES(trace_data, max_size=175, use_time_features=False):
    """extracts featuers from webpage trace
    :return (1) tuple of features and (2) tuple of feature labels
    """
    ...
    if use_time_features:
        ...
        # TIME Features
        FEATURE_LABELS.extend(["intertimestats"+str(i) for i in range(len(intertimestats))])
        ALL_FEATURES.extend(intertimestats)

        FEATURE_LABELS.extend(["timestats"+str(i) for i in range(len(timestats))])
        ALL_FEATURES.extend(timestats)

        FEATURE_LABELS.extend(["avg_per_sec"])
        ALL_FEATURES.append(avg_per_sec)

        FEATURE_LABELS.extend(["std_per_sec"])
        ALL_FEATURES.append(std_per_sec)

        FEATURE_LABELS.extend(['med_per_sec'])
        ALL_FEATURES.append(med_per_sec)

        FEATURE_LABELS.extend(['min_per_sec'])
        ALL_FEATURES.append(min_per_sec)

        FEATURE_LABELS.extend(['max_per_sec'])
        ALL_FEATURES.append(max_per_sec)

        FEATURE_LABELS.extend(['alt_per_sec'] for _ in range(len(alt_per_sec)))
        ALL_FEATURES.extend(alt_per_sec)

        FEATURE_LABELS.extend(['alt_per_sec'])
        ALL_FEATURES.append(sum(alt_per_sec))

        FEATURE_LABELS.extend(['intertimestats_sum'])
        ALL_FEATURES.append(sum(intertimestats))

        FEATURE_LABELS.extend(['timestats_sum'])
        ALL_FEATURES.append(sum(timestats))

    # ORDER & COUNT Features

    FEATURE_LABELS.extend(["number_pkts"+str(i) for i in range(len(number_pkts))])
    ALL_FEATURES.extend(number_pkts)

    FEATURE_LABELS.extend(["thirtypkts"+str(i) for i in range(len(thirtypkts))])
    ALL_FEATURES.extend(thirtypkts)

    FEATURE_LABELS.extend(["stdconc"])
    ALL_FEATURES.append(stdconc)

    FEATURE_LABELS.extend(["avgconc"])
    ALL_FEATURES.append(avgconc)

    FEATURE_LABELS.extend(["avg_order_in"])
    ALL_FEATURES.append(avg_order_in)

    FEATURE_LABELS.extend(['avg_order_out'])
    ALL_FEATURES.append(avg_order_out)

    FEATURE_LABELS.extend(['std_order_in'])
    ALL_FEATURES.append(std_order_in)

    FEATURE_LABELS.extend(['std_order_out'])
    ALL_FEATURES.append(std_order_out)

    FEATURE_LABELS.extend(['medconc'])
    ALL_FEATURES.append(medconc)

    FEATURE_LABELS.extend(['maxconc'])
    ALL_FEATURES.append(maxconc)

    FEATURE_LABELS.extend(['perc_in'])
    ALL_FEATURES.append(perc_in)

    FEATURE_LABELS.extend(['perc_out'])
    ALL_FEATURES.append(perc_out)

    FEATURE_LABELS.extend(['altconc'])
    ALL_FEATURES.extend(altconc)

    FEATURE_LABELS.extend(['altconc_sum'])
    ALL_FEATURES.append(sum(altconc))

    FEATURE_LABELS.extend(['number_pkts_sum'])
    ...
    FEATURE_LABELS.extend(['conc'+str(i) for i in range(len(conc))])
    ALL_FEATURES.extend(conc)

    if use_time_features:
        FEATURE_LABELS.extend(['per_sec' for _ in range(len(per_sec))])
        ALL_FEATURES.extend(per_sec)

    while len(ALL_FEATURES) < max_size:
        ALL_FEATURES.append(0)
        FEATURE_LABELS.append("pad")

    features = ALL_FEATURES[:max_size]
    labels = FEATURE_LABELS[:max_size]
